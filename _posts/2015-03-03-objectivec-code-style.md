---
layout: page
title: "ObjectiveC 代码规范"
category: ios
date: 2015-03-03 19:05:52
---



# 旅游攻略 移动团队 Objective-C 规范指南

这是我们首次将代码规范总结成文，这是一份综合[参考](#参考)的代码规范。对于
...
* [纽约时报 移动团队 Objective-C 规范指南][Introduction_5]
* [Robots & Pencils Objective-C Style Guide][Introduction_6]
...
[Introduction_5]:https://github.com/NYTimes/objective-c-style-guide

[Introduction_6]:https://github.com/RobotsAndPencils/objective-c-style-guide
...
[Condiationals_1]:https://github.com/TGMimes/objective-c-style-guide/issues/26#issuecomment-22074256
...
尽量定义属性来代替直接使用实例变量。除了初始化方法（`init`， `initWithCoder:`，等）， `dealloc` 方法和自定义的 setters 和 getters 内部，应避免通过`self.count = 0`访问实例变量，而应该用`_count = 0`。更多有关在初始化方法和 dealloc 方法中使用访问器方法的信息，参见[这里][Variables_1]。
...
限定符 (`__strong`, `__weak`, `__unsafe_unretained`, `__autoreleasing`) 应该位于最前面，如：`__weak NSString *text`。
...
```objc
static NSString * const TGMAboutViewControllerCompanyName = @"蚂蜂窝网络科技有限公司";
...
```objc
#define CompanyName @"蚂蜂窝网络科技有限公司"
